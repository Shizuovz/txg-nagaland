-- NGE2026 Database Schema (Safe Version)
...
-- Create indexes for better performance (IF NOT EXISTS is not supported for indexes in all PostgreSQL versions)
...
DROP TRIGGER IF EXISTS update_sponsorship_tiers_updated_at ON sponsorship_tiers;
...
DROP TRIGGER IF EXISTS update_visitor_registrations_updated_at ON visitor_registrations;
...
DROP POLICY IF EXISTS "Public read access for colleges" ON colleges;
DROP POLICY IF EXISTS "Public read access for sponsorship tiers" ON sponsorship_tiers;
DROP POLICY IF EXISTS "Allow team registration creation" ON team_registrations;
DROP POLICY IF EXISTS "Allow sponsor registration creation" ON sponsor_registrations;
DROP POLICY IF EXISTS "Allow visitor registration creation" ON visitor_registrations;
DROP POLICY IF EXISTS "Users can read own team registrations" ON team_registrations;
DROP POLICY IF EXISTS "Users can read own sponsor registrations" ON sponsor_registrations;
DROP POLICY IF EXISTS "Users can read own visitor registrations" ON visitor_registrations;
DROP POLICY IF EXISTS "Admin full access to team registrations" ON team_registrations;
DROP POLICY IF EXISTS "Admin full access to sponsor registrations" ON sponsor_registrations;
DROP POLICY IF EXISTS "Admin full access to visitor registrations" ON visitor_registrations;
...
-- Insert initial data (ON CONFLICT DO NOTHING handles existing data)
