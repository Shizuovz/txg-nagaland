...
export const useRegistrationAPI = () => {
  ...
  const submitSponsorRegistration = async (data: SponsorRegistrationData, customSuccessMessage?: string) => {
    setLoading(true);
    setError(null);

    try {
      const response = await registrationAPI.submitSponsorRegistration(data);
      const successMessage = customSuccessMessage || 'Sponsor registration submitted successfully!';
  ...
  const submitVisitorRegistration = async (data: VisitorRegistrationData, customSuccessMessage?: string) => {
    setLoading(true);
    setError(null);

    try {
      const response = await registrationAPI.submitVisitorRegistration(data);
      const successMessage = customSuccessMessage || 'Visitor registration submitted successfully!';
      return handleAPIResponse(response, successMessage);
    } catch (err) {
  ...
  const updateTeamStatus = async (id: string, status: 'pending' | 'approved' | 'rejected' | 'withdrawn') => {
    ...
    try {
      ...
      const errorMessage = (error as Error).message || 'Failed to update team status';
      setError(errorMessage);
      toast.error(errorMessage);
      return false;
    } finally {
  ...
  const updateSponsorStatus = async (id: string, status: 'pending' | 'approved' | 'rejected' | 'withdrawn') => {
    ...
    try {
      ...
      const errorMessage = (error as Error).message || 'Failed to update sponsor status';
      setError(errorMessage);
      toast.error(errorMessage);
      return false;
    } finally {
  ...
  const updateVisitorStatus = async (id: string, status: 'pending' | 'approved' | 'rejected' | 'withdrawn') => {
    ...
    try {
      ...
      const errorMessage = (error as Error).message || 'Failed to update visitor status';
      setError(errorMessage);
      toast.error(errorMessage);
      return false;
    } finally {
  ...
  const updateMediaStatus = async (id: string, status: 'pending' | 'approved' | 'rejected' | 'withdrawn') => {
    ...
    try {
      ...
      const errorMessage = (error as Error).message || 'Failed to update media status';
      setError(errorMessage);
      toast.error(errorMessage);
      return false;
    } finally {
  ...
  const deleteTeamRegistration = async (id: string) => {
    ...
    try {
      const response = await registrationAPI.deleteTeamRegistration(id);
      return handleAPIResponse(response, 'Team registration permanently deleted');
    } catch (err) {
      const errorMessage = err instanceof Error ? err.message : 'Failed to delete team registration';
      setError(errorMessage);
      toast.error(errorMessage);
      return null;
    } finally {
  ...
  const deleteSponsorRegistration = async (id: string) => {
    setLoading(true);
    setError(null);

    try {
      const response = await registrationAPI.deleteSponsorRegistration(id);
      return handleAPIResponse(response, 'Sponsor registration permanently deleted');
    } catch (err) {
      const errorMessage = err instanceof Error ? err.message : 'Failed to delete sponsor registration';
      setError(errorMessage);
      toast.error(errorMessage);
      return null;
    } finally {
  ...
  const deleteVisitorRegistration = async (id: string) => {
    setLoading(true);
    setError(null);

    try {
      const response = await registrationAPI.deleteVisitorRegistration(id);
      return handleAPIResponse(response, 'Visitor registration permanently deleted');
    } catch (err) {
      const errorMessage = err instanceof Error ? err.message : 'Failed to delete visitor registration';
      setError(errorMessage);
      toast.error(errorMessage);
      return null;
    } finally {
  ...
  const deleteMediaRegistration = async (id: string) => {
    setLoading(true);
    setError(null);

    try {
      const response = await registrationAPI.deleteMediaRegistration(id);
      return handleAPIResponse(response, 'Media registration permanently deleted');
    } catch (err) {
      const errorMessage = err instanceof Error ? err.message : 'Failed to delete media registration';
      setError(errorMessage);
      toast.error(errorMessage);
      return null;
    } finally {
